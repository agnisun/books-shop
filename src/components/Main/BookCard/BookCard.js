...
export const BookCard = ({
  ...
  const correctDate =
    publishedDate === "0000" ? "Not available" : publishedDate;
  const correctAuthors = authors ? filterText(authors) : "No author";
  const correctCategories = categories
    ? categories.join(", ")
    : "No categories";
    ...
    <>
      ...
      >
        <Flex flexDirection={"column"}>
          ...
          <Flex justifyContent={"space-between"} fontSize={"14px"} mb={"5px"}>
            <Box textDecoration={"underline"}>{correctCategories}</Box>
            <Box>{correctDate}</Box>
          </Flex>
          <Heading flexBasis={"40px"} mb={"5px"} fontSize={"17px"}>
            {filterText(title, 40)}
          </Heading>
